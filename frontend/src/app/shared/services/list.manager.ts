...
type RequestFn<M extends { oid: string }, Filter extends SearchFilterModel> = (filter: Filter, page: number) => Observable<ArrayResponseI<M>>;
type ResponseFn<M extends { oid: string }, C extends BaseModel> = (responseModels: M[]) => C[];

export abstract class ListManager<M extends BaseModel, C extends BaseModel, Filter extends SearchFilterModel> {
  ...
  public listIsEmpty$: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
  public isLoading$: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
  public length$: BehaviorSubject<number | undefined> = new BehaviorSubject<number | undefined>(undefined);
  ...
  public setResponseFn(prepareResponseData: ResponseFn<M, C>): void {
    this.subs.sink.$dataResponse = this.subscribeToData().subscribe((responseModel: M[]) => {
  ...
  public addSelection<D extends BaseModel>(prepareResponseData: ResponseFn<M, D>): SelectionManager<D> {
  ...
  public removeEntities(oids: string[]): void {
    const entities: M[] = this.entities.filter((item) => !oids.includes(item.oid));
