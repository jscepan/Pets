...
export class SelectionManager<C extends { oid: string; selected?: boolean; transparent?: boolean }> {
  public _items: C[] = [];
  ...
  public getItemById(oid: string): C | undefined {
    return this._items.find((item) => item.oid === oid);
  }
  ...
  private _updateSelectionState(): void {
    ...
    if (this._showSelected) {
      ...
      updatedItems = this._items.map((item: C) => {
        const newItem = { ...item, selectionEnabled: this.enabled, selected: this.isSelected(item.oid) };
        if (this.isSelected(item.oid)) {
          this.select(item.oid, true, newItem);
        }
        return newItem;
      });
    }
  ...
  private _updateSelectionItemStateForSingleSelect(oid: string): void {
    const updatedItems: C[] = this._itemsSubject.getValue().map((item: C) => {
      if (oid === item.oid) {
        ...
        return { ...item, selected: this.isSelected(oid), transparent: false };
      }

      return { ...item, selected: false, transparent: this.isSingleSelectionMode && this.hasSelection() };
    });
